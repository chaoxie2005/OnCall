...
class MilvusClientManager:
    ...
    def connect(self) -> MilvusClient:
        ...
        try:
            ...
            if not self._collection_exists():
                ...
                # 检查索引度量类型是否匹配（例如从 L2 切换到 COSINE）
    ...
    def _ensure_index_metric(self) -> None:
        ...
        indexes = list(getattr(self._collection, "indexes", []))
        if not indexes:
            self._create_index()
            return
        ...
        params = getattr(first_index, "params", {})
        if isinstance(params, dict):
            metric_type = params.get("metric_type")

        if str(metric_type).upper() == self.VECTOR_METRIC_TYPE:
            logger.info(f"索引度量类型匹配: {self.VECTOR_METRIC_TYPE}")
            return

        logger.warning(
            f"检测到索引度量类型不匹配（当前: {metric_type}，目标: {self.VECTOR_METRIC_TYPE}），"
            "将重建 vector 索引。"
        )
