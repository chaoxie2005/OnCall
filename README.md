...
**示例提问**：「帮我查下最近一小时的错误日志」「检索 production 环境 containerd 的异常」
...
OnCall 支持将告警信息推送到飞书群，通过飞书自定义机器人 Webhook 实现。

### 配置飞书机器人

1. 打开目标飞书群 → **设置** → **群机器人** → **添加机器人**，选择 **「自定义机器人」**。
2. 填写机器人名称（如「OnCall 运维告警」），复制 **Webhook 地址**。
3. 将 Webhook 地址配置到 `.env` 或飞书相关配置中。
4. （可选）配置安全设置：自定义关键词、IP 白名单、签名校验。

> 请勿将 Webhook 完整地址提交到 GitHub 或公开博客；若泄露请在飞书侧重置机器人。
